...
```{r ga-income-histogram, fig.cap="Histogram of median household income, Georgia counties"}
library(tidyverse)
options(scipen = 999)
...
```{r ga-income-histogram-newbins, fig.cap = "Histogram with the number of bins reduced to 15"}
ggplot(ga_wide, aes(x = medincE)) + 
...
```{r ga-income-boxplot, fig.cap = "Box plot of median household income, Georgia counties"}
...
```{r first-scatterplot-ga, fig.cap="Scatter plot of median age and median household income, counties in Georgia"}
...
```{r scatterplot-geom-smooth, fig.cap = "Scatter plot with linear relationship superimposed on the graphic"}
...
```{r get-metro-data-show, echo = FALSE}
...
```{r first-plot, fig.cap = "A first bar chart with ggplot2"}
...
```{r second-plot, fig.cap = "An improved bar chart with ggplot2"}
metros %>%
...
```{r polished-plot, fig.cap = "A cleaned-up bar chart with ggplot2"}
...
```{r maine-income-ggplot-1, fig.cap = "A dot plot of median household income by county in Maine"}
...
```{r visualize-error-bars, fig.cap = "Median household income by county in Maine with error bars shown"}
ggplot(maine_income, aes(x = estimate, y = reorder(NAME, estimate))) + 
...
```{r get-deschutes-value-show, echo = FALSE}
style_data(deschutes_value, caption = "Time series of median home values in Deschutes County, OR")
```
...
```{r plot-deschutes-value, fig.cap = "A time series chart of median home values in Deschutes County, OR"}
...
```{r final-deschutes-value-plot, fig.cap = "The Deschutes County home value line chart with error ranges shown"}
ggplot(deschutes_value, aes(x = year, y = estimate, group = 1)) + 
...
style_data(utah, caption = "Age and sex data for Utah from the PEP API")
```
...
```{r first-density-plot, fig.cap = "A density plot using all values in the dataset"}
...
```{r second-density-plot, fig.cap = "A density plot with separate curves for each county"}
...
```{r faceted-housing, fig.cap = "An example faceted density plot"}
ggplot(housing_val2, aes(x = estimate)) +
...
```{r ggridges, fig.cap = "Median home values in Portland-area counties visualized with ggridges"}
library(ggridges)
...
```{r beeswarm, fig.cap = "A beeswarm plot of median household income by most common racial or ethnic group, NYC Census tracts"}
library(ggbeeswarm)
...
```{r geofacet, fig.cap = "Geofaceted population pyramids of US states"}
library(geofacet)
...
```{r plotly-pyramid, fig.cap = "An interactive population pyramid rendered with ggplotly"}
library(plotly)
